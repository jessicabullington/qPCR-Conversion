---
title: "Plate1_E.coli_qPCRConversion"
...
data <- read.csv("Plate1_E.coli_qPCRConversion.csv")
View(data)
...
standardCurvePlot <- plot(CT ~ log10(Amount..Copies.), data = standards, pch=19, ylim=c(20,40), xlim=c(-1,5))
abline(coef(standardCurve))
col = "blue"
legend("topright", legend=c(paste= "slope", round(standardCurve$coefficients[2], 2),
                            paste= "intercept", round(standardCurve$coefficients[1], 2),
...
```{r}
# write csv file
write.csv(unknowns, file = "Plate1_E.coli_ConvertedData.csv", row.names=FALSE, na="")
...
write.csv(data.average, file = "Plate1_E.coli_Averaged.csv", row.names=FALSE, na="")
```
