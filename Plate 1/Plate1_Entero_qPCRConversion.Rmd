...
data <- read.csv("Plate1_Entero_qPCRConversion.csv")
View(data)
...
standardCurvePlot <- plot(CT ~ log10(Amount..Copies.), data = standards, pch=19, ylim=c(20,40), xlim=c(0,5))
abline(coef(standardCurve))
col = "blue"
legend("topright", legend=c(paste= "slope", round(standardCurve$coefficients[2], 2),
                            paste= "intercept", round(standardCurve$coefficients[1], 2),
...
replicates <- replicates[-7,] # This removes rows (A9)
replicates <- replicates[-17,] # This removes rows (B11)
replicates <- replicates[-32,] # This removes rows (D9)
...
```{r}
# write csv file
write.csv(unknowns, file = "Plate1_Entero_ConvertedData.csv", row.names=FALSE, na="")
...
write.csv(data.average, file = "Plate1_Entero_Averaged.csv", row.names=FALSE, na="")
```
